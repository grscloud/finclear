...
const menuItems = computed(() => [
    {
        label: 'アカウント設定', 
        items: [
            //{ label: 'マイプロフィール', icon: 'pi pi-user', command: () => router.push('/profile') },
            { label: '会社情報設定', icon: 'pi pi-building', command: () => router.push('/company-settings') }
        ]
    },
    { separator: true },
    { 
        ...
        command: async () => {
            ...
            // 如果你在 logout 里没有跳转，可以在这里加 router.push('/login')
        } 
    }
]);
...
/* 此时可以使用 :deep() 配合 PrimeVue 的原始类名进行样式穿透 */
...
-->
            ...
            <div class="layout-topbar-menu hidden lg:block">
                ...
                <!-- 下拉菜单：增加 class="profile-menu-panel" 以便后续修改样式 -->
                <!-- AppTopBar.vue -->
                <Menu 
                    ref="profileMenu" 
                    :model="menuItems" 
